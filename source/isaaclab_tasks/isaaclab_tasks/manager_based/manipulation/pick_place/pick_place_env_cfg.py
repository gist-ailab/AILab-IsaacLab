...
class ObjectTableSceneCfg(InteractiveSceneCfg):
    ...
    # target box: will be populated by agent env cfg
    small_klt: RigidObjectCfg | DeformableObjectCfg = MISSING
...
class EventCfg:
    ...
    reset_object_position = EventTerm(
        ...
        mode="reset",
        params={
            "pose_range": {"x": (-0.2, 0.1), "y": (-0.15, 0.1), "z": (0.0, 0.0)},
            # "pose_range": {"x": (-0.001, 0.001), "y": (-0.0025, 0.0025), "z": (0.0, 0.0)},
            "velocity_range": {},
            "asset_cfg": SceneEntityCfg("object", body_names="Object"),
        },
    )

    reset_klt_position = EventTerm(
        func=mdp.reset_root_state_uniform,
        mode="reset",
        params={
            "pose_range": {"x": (-0.07, 0.07), "y": (-0.03, 0.03), "z": (0.0, 0.0)},
            "velocity_range": {},
            "asset_cfg": SceneEntityCfg("small_klt", body_names="small_KLT"),
        },
    )
...
class RewardsCfg:
    ...
    object_goal_tracking = RewTerm(
        ...
        params={"std": 0.3, "minimal_height": 0.02},
        weight=16.0,
    )

    object_goal_tracking_fine_grained = RewTerm(
        func=mdp.object_goal_distance,
        params={"std": 0.05, "minimal_height": 0.02},
        weight=5.0,
    )
